...
public class Activity5 {
    ...
    public void smsTest() {
     ...
     // Tap on the "To" field first (TextView placeholder)
        wait.until(ExpectedConditions.elementToBeClickable(
                AppiumBy.xpath("//android.widget.TextView[@text=\"Type name, phone number or email\"]")
        )).click();

        // Now wait for the actual editable field to appear
        WebElement toField = wait.until(ExpectedConditions.elementToBeClickable(
                AppiumBy.className("android.widget.EditText")
        ));
        
     // Enter phone number
        toField.sendKeys("18282832912");
